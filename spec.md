...
- **imagen `imagen.ts:6,9,12-15`:** `LADO_MAX_IMAGEN=2000`, `CALIDAD_JPEG=0.9`, `BLANCO_UMBRAL=245/MUESTRA=4/RATIO=0.995` (+ `esPaginaNegra` mismo ratio). `normalizarImagen` → recorte fondo + JPEG único; `recortarMargenesBlancos` (línea = bordes lisos `TOL_FONDO=40` con perdón de ruido angosto/tenue `RUIDO_MAX_ANCHO=2/RUIDO_MAX_SALTO=120` + tramo central liso y claro `LUZ_MEDIA=180`, mín 4px; guarda 15% si ralo `AREA_MINIMA=0.15`); error tipado `blanca|ilegible` (negra reuse `blanca`).
